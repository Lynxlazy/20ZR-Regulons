...
```{r}
exprMatr <- read.table('Methylomicrobium_buryatense_5GB1C_count.csv',header = TRUE)
head(exprMatr)
...
```{r}
write.table(weightMat, "weightMat_5GB1.tsv", sep="\t")
```
...
```{r}
write.table(linkList, '5GB1_linkList.tsv', sep='\t')
linkListTop5 <- head(linkList, n = 5000)
write.table(linkList, '5GB1_linkList_5K.tsv', sep='\t')
```
...
write.table(df1, '5GB1C_5K_30clusters.tsv', sep = '\t', row.names = FALSE, quote = FALSE)
